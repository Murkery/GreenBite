// Prüfe sofort beim Laden, ob der User eingeloggt ist
document.addEventListener('DOMContentLoaded', () => {
    const userIsLoggedIn = localStorage.getItem('isLoggedIn');
    const savedName = localStorage.getItem('currentUser');
    
    if (userIsLoggedIn === 'true' && savedName) {
...
function showDashboard(name) {
    ...
    // Canvas-Effekt für den Hintergrund anpassen
...
function logout() {
    ...
    window.location.href = "index.html"; // Explizit zur Startseite leiten
}

// Deine restliche Canvas-Logik...

function continueAsGuest() {
    // Wir speichern "true" für den Login und "Guest" als Name
    localStorage.setItem('isLoggedIn', 'true');
    localStorage.setItem('currentUser', 'Guest');
    
    // Dann wie gewohnt die Bereiche umschalten
    document.getElementById('auth-section').style.display = 'none';
    document.getElementById('app-section').style.display = 'block';
    document.getElementById('display-name').innerText = "Guest";
}

// 1. CANVAS & BASIS SETUP
...
class Particle {
    constructor(svgString = null) {
        ...
        if (svgString) {
            ...
            this.path = pathMatch ? new Path2D(pathMatch[1]) : null;
            this.isSVG = true;
        } else {
...
function toggleAuthMode() {
    ...
    document.getElementById('toggle-link').innerText = isLoginMode ? "Don't have an account? Sign up" : "Already have an account? Login";
}

function handleAuth() {
    const user = document.getElementById('username').value;
    const pass = document.getElementById('password').value;
    if (!user || !pass) return alert("Fülle alles aus!");
    const res = validateAuth(user, pass, isLoginMode);
    if (res.success) { startApp(user); } else { alert(res.message); }
}

function continueAsGuest() { startApp("Guest"); }
...
function startApp(name) {
    ...
    // WICHTIG: Canvas transparent machen, aber NICHT auf display:none setzen
    canvas.style.opacity = "0";
    particles = []; // Gemüse entfernen
}

// 3. APP INTERAKTION (Buttons & Hover)
const buttonIcons = {
    "Find Recipes": [
        '<svg viewBox="0 0 24 24"><path d="M12 6v2M9 5l1 3M15 5l-1 3"/></svg>', 
        '<svg viewBox="0 0 24 24"><path d="M14 4l-9 9c-1.5 1.5-1.5 3.5 0 5s3.5 1.5 5 0l9-9c1-1 1-2.5 0-3.5s-2.5-1-3.5 0z"/></svg>'
    ],
    "My Tracking": [
        '<svg viewBox="0 0 24 24"><path d="M3 3v18h18"/><path d="M7 14l4-4 4 4 6-6"/></svg>',
        '<svg viewBox="0 0 24 24"><path d="M12 20a8 8 0 100-16 8 8 0 000 16z"/></svg>'
    ],
    "My Collection": [
        '<svg viewBox="0 0 24 24"><path d="M4 19V5a2 2 0 012-2h13.4a.5.5 0 01.49.4v15.2a.5.5 0 01-.49.4H6a2 2 0 01-2-2z"/></svg>',
...
document.querySelectorAll('.menu-btn').forEach(btn => {
    btn.addEventListener('mouseenter', () => {
        ...
        const icons = buttonIcons[text];
        if (!icons) return;

        canvas.style.opacity = "0.4";
        particles = [];
        for (let i = 0; i < 20; i++) {
            particles.push(new Particle(icons[Math.floor(Math.random() * icons.length)]));
        }
    });
...
document.querySelectorAll('.menu-btn').forEach(btn => revealObserver.observe(btn));
...
document.querySelectorAll('.menu-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (text === "Find Recipes") {
            window.location.href = "rezepte.html";
        } else if (text === "My Collection") {
            window.location.href = "collection.html";
        } else if (text === "My Tracking" || text === "Tracker") { 
            // Hier die Verknüpfung zu deiner neuen Datei
            window.location.href = "tracker.html";
        }
    });
});

    // 2. HINTERGRUND-EFFEKT (Beim Hovern)
    btn.addEventListener('mouseenter', () => {
        const text = btn.innerText.trim();
        const icons = buttonIcons[text];
        
        if (icons) {
            canvas.style.opacity = "0.4"; // Canvas einblenden
            particles = []; // Altes Gemüse/Icons sofort löschen
            
            // Neue Partikel nur mit den passenden Icons erstellen
            for (let i = 0; i < 20; i++) {
                particles.push(new Particle(icons[Math.floor(Math.random() * icons.length)]));
            }
        }
